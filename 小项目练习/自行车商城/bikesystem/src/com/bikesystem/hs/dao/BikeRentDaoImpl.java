...
public class BikeRentDaoImpl extends BaseSystemDao implements IBikeRentDao{
	...
	@Override
	public BikeRent queryBikeRentByBrid(int brid) {
		String sql="select * from bike_rent where brid=? ";
		try{
			return qr.query(sql, new BeanHandler<BikeRent>(BikeRent.class),brid);
		}catch(Exception e){
			e.printStackTrace();
		}
		return null;
